...
export async function POST(req) {
  try {
    ...
    const store = await authSeller(userId);
    if (!store) return new NextResponse("Store not found or not approved", { status: 404 });
    ...
    if (!name || !description || !mrp || !price || !category || !imageFiles.length)
      return new NextResponse("Missing required fields", { status: 400 });

    // Upload des images vers ImageKit
    const uploadedImages = [];
    const folderName = store.username ? `/stores/${store.username}/products` : `/stores/${store.id}/products`;
    ...
    for (const imageFile of imageFiles) {
      if (imageFile && imageFile.name !== "undefined") {
        ...
        const fileName = `product_${timestamp}_${Math.random().toString(36).substring(2, 9)}.${fileExtension}`;
...
export async function PUT(req) {
  try {
    ...
    const name = formData.get("name");
    const description = formData.get("description");
    const mrp = formData.get("mrp") ? parseFloat(formData.get("mrp")) : undefined;
    ...
    for (const imageFile of imageFiles) {
      if (imageFile && imageFile.name !== "undefined") {
        ...
        const timestamp = Date.now();
        const fileExtension = imageFile.name.split(".").pop();
        const fileName = `product_${store.username}_${timestamp}_${Math.random()
          .toString(36)
        ...
        const uploadResponse = await imagekit.upload({
          file: buffer,
          fileName,
          folder: `/stores/${store.username}/products`,
        });
