...
export async function POST(req) {
  ...
  const stripe = new Stripe(process.env.STRIPE_SECRET_KEY, {
    apiVersion: "2024-06-20", // toujours mettre la version la plus récente stable
  });
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: items.map((item) => ({
      price_data: {
        ...
        product_data: {
          name: item.name, // il faut envoyer le nom du produit
        },
        unit_amount: Math.round(item.price * 100), // Stripe attend un montant en CENTIMES
      },
      quantity: item.quantity,
    })),
    success_url: `${origin}/orders?status=success`,
    cancel_url: `${origin}/cart?status=cancel`,
    metadata: {
      userId,
      orderIds: JSON.stringify(orderIds), // pour récupérer les commandes côté webhook Stripe
    },
  });
