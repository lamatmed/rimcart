...
export default function CreateStore() {
  ...
  const fetchSellerStatus = async () => {
    ...
    try {
      ...
      console.error("fetchSellerStatus error:", error);
      setAlreadySubmitted(false);
      setMessage("");
    } finally {
      setLoading(false);
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      // Redirection directe vers /store
      router.push("/store");
    } catch (error) {
      ...
      if (error.response) {
        ...
        else if (status === 400) toast.error(data.message || "Missing required fields");
        else if (status === 404) toast.error("Store not found");
