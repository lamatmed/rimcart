...
/**
 * Middleware pour vérifier si l'utilisateur est authentifié et a un store actif
 * @param {Request} req - La requête entrante
 * @param {boolean} requireActiveStore - Si true, vérifie que le store est actif
 * @returns {Object} { userId, store, nextResponse } ou une réponse d'erreur
 */
export async function authSeller(req, requireActiveStore = true) {
  try {
...
/**
 * Middleware pour les routes API qui nécessitent un vendeur authentifié
 * Utilisation: export const GET = withAuthSeller(handler);
 */
